 ...
 * Passes team names for accurate lineup generation.
 */
export async function fetchDemoData(
  ...
  const url = `${API_BASE}/api/football/demo/${matchId}${queryString ? '?' + queryString : ''}`;
 ...
 * 
 * Team names are used to generate realistic demo data with correct lineups.
 */
export async function fetchFootballData(
  ...
  // - lineups must have at least home team data
  // - stats must have possession data (key indicator)
  // - events can be empty (early in match) but must be an array
  const hasRealData = lineups && stats && stats.possession && events &&
    (lineups.home?.startXI?.length ?? 0) > 0;
  ...
  // Fallback to demo data with real team names
